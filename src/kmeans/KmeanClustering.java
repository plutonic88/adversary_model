...
public class KmeanClustering {
	...
	private static  boolean RAND_POINTS_FROM_OBSERVATION = false; //RMFO
	private static  boolean RAND_ACTION_INIT_TO_CLUSTERS = true; // reic
...
/*	public static void doExPeriemnt(int numberofclusters)
	...
	/**
	 * cluster users using different distance metric
	 * @param numberofclusters
	 * @param player
	 * @param mg
	 * @return
	...
	public static List<Integer>[] clusterUsersV2(int numberofclusters, double[][] examples )
	{
		...
		//int opponent = 1^player;
		//final int   RUNNING_FOR = 20;
		int featurelength = examples[0].length;
		int numberofexamples = examples.length;
		//double[] extreampayoffs = mg.getExtremePayoffs();
		List<Double[]>[] clusters = new List[numberofclusters]; // create an array of list. Each list will contain arrays of double
		double[][] clusterpoints = new double[numberofclusters][featurelength];  // cluster points for each cluster
		double[][] oldclusterpoints = new double[numberofclusters][featurelength];  
		double[][] distancefromcluster = new double[numberofclusters][featurelength];
		int runningInstance = 0;
		double[] sumofdifferences = new double[numberofclusters];  //store the sum of differences for clusters
		double[] maxdifference = new double[numberofclusters]; 
		double[] squaredrootdifference = new double[numberofclusters];
		...
		if(KmeanClustering.isRandPointsFromObservation()) // implemented only for two players
		{
			...
			for(int i=0; i< numberofclusters; i++)
			{
				...
				while(true)
				{
					if(points.size()==0)
					{
						exampleforclusterpoint = randInt(0, numberofexamples-1); // choose an action randomly
						points.add(exampleforclusterpoint);
						break;
					}
					else
					{
						exampleforclusterpoint = randInt(0, numberofexamples-1); // choose an action randomly
						if(!points.contains(exampleforclusterpoint))
						{
							points.add(exampleforclusterpoint);
							break;
						}
				...
				{
					clusterpoints[i][index] = examples[exampleforclusterpoint][index];
		...
		/*
		 * assigns random examples to each clusters
		 */
		if(KmeanClustering.isRandActionInitToClusters())
		{
			 ...
			 * then calculate the mean
			 */
			ArrayList<Integer> unassignedexamples = new ArrayList<Integer>();

			for(int i=0;i<numberofexamples;i++)
			{
				unassignedexamples.add(i);
			}
			// iterate through all the actions and assign to a cluster randomly
			for(int clusterindex=0; clusterindex<numberofclusters; clusterindex++)
			{

				for(int j=0; j<(numberofexamples/numberofclusters); j++) //slot, it ensures that every cluster has equal number of actions for random partition
				{

					if(unassignedexamples.size()>1)
					{
						int chosenexampleindex = randInt(1, unassignedexamples.size()) -1;
						int z = unassignedexamples.get(chosenexampleindex);
						assignToCluster(clusters, unassignedexamples.get(chosenexampleindex), clusterindex, featurelength, examples);
						//	Logger.log("\n random paritiion<<<>>>>> Action "+ z+ " of player "+ player+" is assigned to cluster "+i, false);
						unassignedexamples.remove(chosenexampleindex);
					}
					else if(unassignedexamples.size() ==1)
					{
						assignToCluster(clusters, unassignedexamples.get(0), clusterindex, featurelength, examples);
						int x = unassignedexamples.get(0);
						//	Logger.log("\n random paritiion<<<>>>>> Action "+x + " of player "+ player+" is assigned to cluster "+i, false);
						unassignedexamples.remove(0); //remove the last element
						break;
					}
			...
			//check if there are any actions remained unassigned
			if(unassignedexamples.size() != 0)
			{
				for(Integer x: unassignedexamples)
				{
					int a = numberofclusters-1;
					assignToCluster(clusters, x, numberofclusters-1, featurelength, examples); // assign all the remainning actions to the last cluster.
					//	Logger.log("\n random paritiion<<<>>>>> Action "+ x+ " of player "+ player+" is assigned to cluster "+a, false);
			...
			calculateClusterMeanD(clusters, clusterpoints, numberofclusters, examples, featurelength);
		...
		while(true)
		{
			...
			for(int i = 0; i < numberofexamples; i++)
			{
				for(int j = 0; j < featurelength; j++)
				{
					...
					double tmpfeaturevalue = examples[i][j];
					for(int k =0; k<numberofclusters; k++)
					{
						/*
						 * calculate the differences of payoffs for each cluster points 
						...
						if(KmeanClustering.isDistMetricLine())
						{
							...
							if((tmpfeaturevalue < 0 && clusterpoints[k][j] < 0) ||  (tmpfeaturevalue >=0  && clusterpoints[k][j] >= 0))

							{
								distancefromcluster[k][j] = Math.abs((Math.abs(tmpfeaturevalue) - Math.abs(clusterpoints[k][j])));
							}
							else if((tmpfeaturevalue >= 0 && clusterpoints[k][j] < 0))
							{
								distancefromcluster[k][j] = (tmpfeaturevalue + Math.abs(clusterpoints[k][j]));
							}
							else if((tmpfeaturevalue < 0 && clusterpoints[k][j] >= 0))
							{
								distancefromcluster[k][j] = (clusterpoints[k][j]  + Math.abs(tmpfeaturevalue));
							}
						}
						/*
						 * calculate euclidean distance: first take the squares of difference....
						...
						{
							//	Logger.log("\n entered DistMetricEuclidean() ", false);
							distancefromcluster[k][j] = (clusterpoints[k][j]  - (tmpfeaturevalue));
							distancefromcluster[k][j] = distancefromcluster[k][j] * distancefromcluster[k][j];
						}
				...
				//Logger.log("\nAction "+ a+" is assigned to cluster :"+minindex , false);
				//System.out.println("\nIteration: "+ runningInstance + "Assigning cluster points ");
				assignToCluster(clusters, i, minindex, featurelength, examples);
				//assignToCluster(clusters, i, numberofclusters-1, featurelength, examples);
		...
		PrintWriter pw = null;
		String actions = "K"+numberofclusters+"-"+0+","+featurelength+",";
		try {
			pw = new PrintWriter("abstractions"+".txt");
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	*/
			...
			int assignedcluster, int numberoffeatures, double[][] examples)
	{
		...
		//int opponent = 1^player;
		//int oppnumaction = mg.getNumActions(opponent);
		Double[] tupleincluster = new Double[numberoffeatures+1]; // +1 for the example index
		tupleincluster[0] = (double)exampletoassign; //the action in the first index
		/*
		...
		for(int p = 0; p< numberoffeatures; p++)
		{
			...
			tupleincluster[p+1] = examples[exampletoassign][p];
